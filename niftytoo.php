...
Version 1.6.9 - Last modified 2023-03-19
*/
...
function nifty2_convert($out) {
	...
	if (strpos($out, '://') !== false) {
		$out = preg_replace_callback(
			// 1:scheme+authority                         +domain               \2:path                                   \3:query+fragment
			'!(https?://)([a-zA-Z0-9~@#%:\.\-\+_=]{1,256}\.[a-zA-Z0-9()]{1,6}\b)([a-zA-Z0-9()~\!@#$%&;,\.\/\-\+_=]{1,256})(\??[a-zA-Z0-9()~\!@#$%&:;,\.\/\-\+_=]*)!i',
			function ($m) {
				$extra = $m[4];
				if (strlen($m[4]) > 70) {
					$extra = empty($m[3]) ? '' : mb_substr($m[4], 0, 50).'[...]';
				}
				return '<a href="'.nifty2_nice_url($m[1].$m[2].$m[3].$m[4]).'">'.$m[1].$m[2].$m[3].$extra.'</a>';
			},
			$out
		);
	}
	...
	if (strpos($out, ']') !== false) {
		...
		// Are we done, or are there more tags to process?
		if (strpos($out, ']') !== false) {
		...
		// Any leftover tags are the expensive callback replacements.
		if (strpos($out, ']') !== false) {
	...
	// Smilies (do last, as $out must be fully HTML encoded)
	if ($cfg['smilies'] != 'no') {
