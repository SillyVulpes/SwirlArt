...
Version 1.6.12 - Last modified 2025-07-27
*/
...
<?php
	...
	for ($current_post = 0; $current_post < $rownum; $current_post++) {
		...
		// Null fixes
		if (!isset($outerrow['title'])) $outerrow['title'] = '';
...
<?php
		...
		// Retouch
		if ($flags['D']) {
			if ($outerrow['usrname'] == $user['usrname'] || !empty($outerrow['password'])) {
				switch ($outerrow['datatype']) {
					case '0':
						$use_app = 'paintBBS.php?';
						break;
					case '1':
				...
				if ($outerrow['usrname'] != $user['usrname']) {
					$use_app = 'retouch.php?';
				}
		...
		if ($outerrow['threadlock'] == '0' || $flags['mod']) {
			if ($cfg['guest_post'] == 'yes' || !empty($user['usrname'])) {
				?>
...
<?php
		...
		if ($outerrow['edittime'] > 10 ) {
			...
			$edit_m = floor(intval($outerrow['edittime'] / 60) % 60);
			$edit_h = floor(intval($outerrow['edittime'] / 3600) % 60);
...
<?php
		...
		// Block adult images
		if ($flags['mod'] || $outerrow['usrname'] == $user['usrname']) {
...
<?php
		...
		// Paint type
		if ($outerrow['datatype'] == '0' || $outerrow['datatype'] == '6') {
...
<?php
		...
		if ($outerrow['animation'] == '1' && $outerrow['datatype'] != '4' && $outerrow['datatype'] != '5') {
			$viewani_size = array('75', '175');

			if ($outerrow['datatype'] == '1') {
...
<?php
		...
		<div class="commentmain">
			<div class="commentheader">
				...
				<?php
					if ($outerrow['usrname'] == $user['usrname'] || $flags['mod'])
...
<?php
		...
		if (!empty($outerrow['avatar'])) {
			if ($cfg['use_avatars'] == 'yes' && strtolower($user['usrname']) != 'screenshot') {
...
<?php
		...
		while ($i2 < $rownum2) {
			...
			// Null fixes
			if (!isset($innerrow['url'])) $innerrow['url'] = '';
...
<?php
			if ($innerrow['usrname'] == 'Guest'){
				...
				if (!empty($user['usrname']) && !empty($innerrow['email'])) {
					?>
...
<?php
			...
			// Edit comment
			if ($innerrow['usrname'] == $user['usrname'] || $flags['mod']) {
?>
...
<?php
			...
			// Delete comment
			if ($innerrow['usrname'] == $user['usrname'] || $flags['admin']) {
				// Confirm
...
<?php
		...
		if (!empty($innerrow['avatar'])) {
			if ($cfg['use_c_avatars'] == 'yes' && $cfg['use_avatars'] == 'yes' && $innerrow['usrname'] != 'Guest' && strtolower($user['usrname']) != 'screenshot') {
