...
function sql_check($result, $extra = 'query') {
	if (!$result) {
		echo "\n<p>\n";
		echo db_error();
		echo "<br />\n";

		echo '['.$extra.']: '.db_print_error(true);
		echo "\n</p>\n";
...
EOF;
	...
	// Test database connection (forced) before going any further.
	include 'db_layer.php';
	$dbconn = db_open(true);
	if (!$dbconn) {
		web_line( t('err_mysqlconnect'));
...
<?php
	...
	if (!$glob['debug']) {
		...
		*/
		...
		// UTF-8 compliance marker.  1=partial conversion, 2=converted, 3=installed
