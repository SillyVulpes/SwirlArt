...
if ($username != $OekakiU && $flags['admin']) {
	...
	if ($result) {
		if (db_num_rows($result)) {
			...
			report_err( t('mem_not_found', stripslashes($username)));
		}
	} else {
		// It's serious to have SQL bomb here
		w_log_sql('edit_avatar:'.__LINE__);
		report_err( t('prof_not_ret', stripslashes($username)));
	}
} else {
	// Edit own profile
	$username = $OekakiU;
...
if (w_gpc('action') == 'do') {
	...
	if (w_gpc('del')) {
		...
		if ($type != 'png' && $type != 'jpg' && $type != 'gif' && $type != 'webp' && $type != 'jp2' && $type != 'jxl') {
		...
		$filename = md5(stripslashes($username).strval(rand() )).'.'.$type;
...
<?php if ($flags['admin'] && $OekakiU != $username) { ?>
	...
	<td class="pinfoenter" valign="top">
		<?php echo w_html_chars(stripslashes($username));?> 
	</td>
	</tr>
...
<?php } ?>
	...
	<td class="pinfoenter" valign="top">
		<form name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">
			...
			<input name="username" type="hidden" value="<?php echo w_html_chars(stripslashes($username));?>" />
