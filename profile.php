...
Version 1.6.12 - Last modified 2025-08-07
*/
...
// SQL preparation
$count = db_query("SELECT COUNT(ID) FROM `{$db_mem}oekaki` WHERE usrname='$name'");
$exists = intval(db_result($count, 0));
if ($exists < 1) {
	report_err('Invalid username', true);
}
...
if ($flags['mod']) {
	...
	if ($post_result || $com_result) {
		if (!isset($com_row['postdate'])) $com_row['postdate'] = 0;
		if (!isset($post_row['postdate'])) $post_row['postdate'] = 0;

		if ($com_row['postdate'] > $post_row['postdate']) {
			$the_IP = !empty($com_row['IP']) ? t('ip_by_comment', $com_row['IP']) : '';
			if (!empty($com_row['hostname']) || $com_row['hostname'] != 'invalid') {
				$the_host = $com_row['hostname'];
			}
		} else if ($post_row) {
			$the_IP = !empty($post_row['IP']) ? t('ip_by_picture', $post_row['IP']) : '';
...
<div class="pheader">
	<?php
	tt('profile_title', w_html_chars(stripslashes($name)), $name_gender);?> 
</div>
...
?>
	...
	<td class="pinfoenter" valign="top">
<?php
		$the_date = isset($row['age']) ? $row['age'] : '';
...
if (isset($row['IRCServer']) && strlen($row['IRCServer']) > 7) { ?>
<!-- IRC -->
<div class="pheader">
...
while ($i < $rownum) {
	...
	// Get title
	$row['title'] = isset($row['title']) ? trim($row['title']) : '';
	$title = $row['title'];
	if (empty($title)) {
	...
	if (empty($row['title'])) {
		...
		$lightbox_title = 'title="&quot;'.w_html_chars($title).'&quot; by '.w_html_chars($row['usrname']).'"';
	}
