...
Wacintaki Poteto modifications Copyright 2004-2025 Marc "Waccoon" Leveille
https://www.NineChime.com/products/
Version 1.6.12 - Last modified 2025-08-25
*/
...
// Mass OPMail
if ($action == 'massmail') {
	$send_to = w_limit(w_gpc('send_to'));
	$body    = w_gpc('body');
	$subject = w_limit(w_gpc('subject'));
	...
	if ($flags['owner']) {
		...
		if (!empty($send_to) && !empty($user['usrname'])) {
			if (empty($subject)) {
...
// Lost Password Send
if ($action == 'pretrieve') {
	$username = w_limit(w_gpc('username'));
...
if ($action == 'pass_reset') {
	$usrname2  = w_limit(w_gpc('usrname2'));
...
if ($action == 'mailsend') {
	$body     = w_gpc('body');
	$subject  = w_limit(w_gpc('subject'));
	$reciever = w_limit(w_gpc('reciever'));
	$MID      = w_gpc('MID', 'i');
	$reply    = w_gpc('reply', 'i');

	if (!empty($user['usrname']) && $flags['G']) {
		if (empty($subject)) {
			report_err( t('functions_err1'));
		}
...
if ($action == 'register') {
	...
	$age      = decode_birthday(w_gpc('age_year'), w_gpc('age_month'), w_gpc('age_day'));
	$pass     = w_gpc('pass');
	$pass2    = w_gpc('pass2');
	...
	if ($bad = badChars(stripslashes($username), 'name')) {
...
if ($action == 'adduser') {
	...
	if ($accept) {
		...
		w_log(WLOG_REG, '$l_accept_f`'.$theflags.'`', stripslashes($usrname2));
...
if ($action == 'editprofile') {
	...
	$gender     = w_limit(w_gpc('gender'), 20);
	$location   = w_limit(w_gpc('location'));
	$url        = w_gpc('url', 'url');
	$urltitle   = w_limit(w_gpc('urltitle'));
	$aim        = w_limit(w_gpc('aim'));
	$icq        = w_limit(w_gpc('icq'));
	$msn        = w_limit(w_gpc('msn'));
	$yahoo      = w_limit(w_gpc('yahoo'));
	$ircserver  = w_limit(w_gpc('ircserver'));
	$ircnick    = w_limit(w_gpc('ircnick'));
	$ircchan    = w_limit(w_gpc('ircchan'));
	...
	$username2  = w_limit(w_gpc('username2'));
	...
	// Update log.
	if ($my_admin_flag) {
		w_log(WLOG_EDIT_PROFILE, '$l_prof_up', stripslashes($username2));
	}
	...
	if (!empty($passwd) && !empty($passwdnew)) {
		...
		if (!empty($trim_passwd) && w_password_verify($oldpass, $OekakiPass)) {
			if ($passwd == $passwdnew) {
				...
				if ($my_admin_flag) {
					w_log(WLOG_PASS_RESET, '$l_reset_admin', $user['usrname']);
				} else {
					w_log(WLOG_PASS_RESET, '$l_reset_mem', stripslashes($username2));
				}
...
if ($action == 'upload') {
	...
	if (!empty($_POST['usrname']) && $flags['mod']) {
		$upload_name = w_limit(w_gpc('usrname'));
	} else {
		$upload_name = $OekakiU;
	}
	...
	if ($type == 'bmp') {
		...
		report_err('JPEG2000 is not supported, as the thumbnail generator cannot reat it.');
	} elseif ($type == 'jxl') {
		report_err('JPEG-XL is not yet supported, as the thumbnail generator cannot reat it.');
	} elseif ($type == 'psd') {
	...
	} elseif ($type != 'png' && $type != 'jpg' && $type != 'gif' && $type != 'webp') {
		report_err('Picture is an unsupported filetype.');
	}


	// Check dimentions for sanity
	$p_sizes = @GetImageSizeFromString($buffer);
	if ($p_sizes !== false) {
...
if ($action == 'res_msg') {
	...
	if (empty($user['usrname']) || empty($picno)) {
		all_done();
	}
	...
	// Check all passwords
	if ($user['usrname'] == $old_pic_info['usrname'] || w_transmission_verify($old_pic_info['password'], $old_pass_hash)) {

		// Special case for public pictures.  Cannot edit other's public pictures
		if ($user['usrname'] != $old_pic_info['usrname'] && $old_pic_info['password'] == 'public') {
			report_err('Direct public retouch not allowed (possible bug!)');
		}

		// Check immunity flag for addition
		if ($cfg['draw_immune'] == 'yes' && !$flags['I']) {
			if ($user['usrname'] == $old_pic_info['usrname']) {
...
if ($action == 'add') {
	...
	if (!empty($user['usrname'])) {
		...
		if ($bad = badChars(stripslashes($name), 'name')) {
			report_err( t('name_invalid_chars', make_list($bad, ', ', '"')) );
		}

		// Validate URL
		if (empty($url) || strlen($url) < 8) {
			$url = '';
		}
...
if ($action == 'del') {
	...
	if ($row['usrname'] == $user['usrname'] || $flags['admin']) {
		include 'paintsave.php';
...
if ($action == 'delr') {
	...
	if ($result) {
		...
		if ($flags['admin'] || $user['usrname'] == $usrname) {
			require 'paintsave.php';
			kill_picture_slot($picno);
...
if ($mode == 'block' || $mode == 'unblock') {
	...
	if ($flags['mod'] || $pic['usrname'] == $user['usrname']) {
		// Toggle adult status
		$block = (int) $pic['adult'];
...
if ($mode == 'wip_pic') {
	...
	if ($flags['mod']) {
		...
		if ($result) {
			...
			$message = t('to_wip_admin_note', $user['usrname'], $cfg['safety_storetime']);
