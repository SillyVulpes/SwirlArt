...
// SQL
$result = db_query("SELECT `PIC_ID`, `usrname`, `datatype`, `animation`, `title`, `edittime`, `px`, `py` FROM `{$db_p}oekakidta` WHERE ID_2=".$recno);
if (!$result) {
...
$have_animation = true;
if (!isset($row['animation']) || $row['animation'] != 1) {
...
if ($row['edittime'] > 10 ) {
	...
	$edit_m = floor(intval($row['edittime'] / 60) % 60);
	$edit_h = floor(intval($row['edittime'] / 3600) % 60);
...
?>
	...
	<?php tt('viewani_files'); echo ': '.strval($file_size).'KB';?> 
</div>
<?php
}
