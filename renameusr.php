...
Version 1.6.10 - Last modified 2023-07-11
*/
...
if (isset($_POST['oldname'])) {
	$result = db_query("SELECT usrname FROM `{$db_mem}oekaki` WHERE usrname='{$oldname}'");
...
$newname_okay = false;
if (!empty($newname)) {
	$result = db_query("SELECT usrname FROM `{$db_mem}oekaki` WHERE usrname='{$newname}'");
...
if ($action == 'perform' && !empty($oldname)) {
	$result = db_query("SELECT ID, usrname, `rank` FROM `{$db_mem}oekaki` WHERE usrname='{$oldname}'");
...
?>
			...
			<td class="infoenter" valign="top">
				<input name="newname" type="text"<?php if (!empty($newname)) echo 'value="'.w_html_chars(stripslashes($newname)).'"';?> size="40" class="txtinput" />
			</td>
			</tr>
...
if ($action == 'confirm') {
	...
	<form id="form11" method="post" action="renameusr.php">
		...
		<input type="hidden" name="oldname" value="<?php echo w_html_chars(stripslashes($oldname));?>" />
		<input type="hidden" name="newname" value="<?php echo w_html_chars(stripslashes($newname));?>" />
		...
		<div class="infotable">
			...
			<td class="infoenter" valign="top">
				<?php echo w_html_chars(stripslashes($oldname));?> 
			</td>
			</tr>
			...
			<td class="infoenter" valign="top">
				<?php echo w_html_chars(stripslashes($newname));?> 
			</td>
			</tr>
