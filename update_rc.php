<?php
/*
Wacintaki Poteto - Copyright 2011-2023 Marc "Waccoon" Leveille
			...
			if ($type === 'utf8') {
				foreach ($keyvals as $key => $val) {
					if (check_utf8_valid($val)) {
						...
						// PHP<8.2: $keyvals[$key] = utf8_encode($keyvals[$key]);
						$keyvals[$key] = mb_convert_encoding($keyvals[$key], 'UTF-8', 'ISO-8859-1');
					}
				}
			} elseif ($type === 'big5') {
				foreach ($keyvals as $key => $val) {
