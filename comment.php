...
Version 1.6.10 - Last modified 2023-07-19
*/
...
// No guests?
if (empty($user['usrname']) && $cfg['guest_post'] != 'yes') {
	all_done();
}
...
// Check ownership
if ($mode == 'res_msg' && !$flags['owner'] && $user['usrname'] != $row['usrname'] && empty($row['password'])) {
...
<?php
	$print_pass = '';
	if (isset($import_info['usrname']) && $import_info['usrname'] == $user['usrname']) {
		$print_pass = $import_info['password'];
	}

	if (!empty($import_info['password']) && $import_info['usrname'] != $user['usrname']) {
