...
Version 1.6.10 - Last modified 2023-07-12
*/


function verify_edit($testnum, $hash = '') {
	// Need DB, username, password
	global $dbconn, $db_p, $db_mem, $OekakiU, $OekakiPass, $user;
	...
	if ($user['usrname'] != $row['usrname'] || $OekakiPass != $row2['usrpass']) {
			...
			switch ($p_ext) {
				...
				case 'webp':
					if (!function_exists('imagecreatefromwebp')) {
						// No WEBP support!  Nothing we can do.
						// We don't want to throw an error to members, so return non-zero.
						return 1;
					} else {
...
function decode_upload_err($errno) {
	if ($errno > 0) {
		switch ($errno) {
			case UPLOAD_ERR_INI_SIZE:
				return 'Uploaded file exceeds the UPLOAD_MAX_FILESIZE directive in php.ini.';
			case UPLOAD_ERR_FORM_SIZE:
				return 'Uploaded file exceeds the MAX_FILE_SIZE directive in the HTML form.';
			case UPLOAD_ERR_PARTIAL:
				return 'Uploaded file truncated.';
