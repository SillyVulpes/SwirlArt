...
/*
Wacintaki Poteto -  Copyright 2004-2025 Marc "Waccoon" Leveille
https://www.NineChime.com/products/
Version 1.6.12 - Last modified 2025-09-01

Requires:
	$OekakiU, $OekakiPass, $OekakiID.

	$OekakiID and $OekakiPass must be preset if they are not cookies.
	Both are filtered and escaped by default, and SQL safe.

	$OekakiU is not a cookie, and is supported for legacy reasons.
	$OekakiU is escaped and SQL safe, otherwise use $user['usrname'].
...
$version       = '1.6.12';
$dbconn        = false;
$db_mem        = ''; // Member prefix
$db_p          = ''; // Data prefix
//$OekakiID    = ''; // Do not set/test here
//$OekakiPass  = ''; // Do not set/test here
$OekakiU       = ''; // Always escaped
$mode          = '';
$action        = '';
$start_time    = ''; // Page load/render timer
$language      = '';
$template_name = '';
$cssinclude    = ''; // Path for HTML style tag
$address       = 'invalid'; // IP4 or IP6 as string
$hostname      = '';
$smilies_group = '';
$p_pic    = ''; // Picture prefix (full sized)
$t_pic    = ''; // Thumbnail prefix
$r_pic    = ''; // Reduced prefix (scaled size)
$p_prefix = '';
$t_prefix = '';
$r_prefix = '';
...
$sort    = w_gpc('sort', 'i+');
$artist  = w_limit(w_gpc('artist'));
$a_match = w_limit(w_gpc('a_match'));
$t_match = w_limit(w_gpc('t_match'));
...
// Note: DES hashing removed in PHP 8.  Blowfish is minimum for PHP >= 5.5.
// Password (0=DES, 1=DES Extended, 2=Blowfish)
...
if (!empty($OekakiPass) && !empty($OekakiID) ) {
	...
	if ($flags['member'] == 1) {
		$OekakiID = w_toalpha($user['ID']);
		$OekakiU  = addslashes($user['usrname']);
...
if (!$quiet_mode) {
	...
	if ($flags['member']) {
		...
		if (strtolower($user['usrname']) == 'screenshot') {
			$user['thumbview'] = 0;
		}
	} else {
		$user['thumbview'] = $cfg['use_thumb'];
