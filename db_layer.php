...
/*
Wacintaki Poteto - Copyright 2014-2023 Marc "Waccoon" Leveille
https://www.NineChime.com/products/
Version 1.6.10 - Last modified 2023-07-10
...
// Highres Timer for Windows.  Microsecond accuracy only needed for SQL benchmarks.
...
function db_open($force = false) {
	...
	// Use non-exeption error reporting, which respects debug flags.
...
function db_query($sql) {
	...
	if (HR_TIMER) {
		// $q_time must be microsecond float
		$q_time = (hrtime(true) - $q_start) / 1e+9;
	} else {
...
function db_error_array() {
	...
	return array(
		'sql'   => $errs[0],
		'trace' => $errs[2],
		'num'   => $dbconn ? @mysqli_errno($dbconn) : $errs[3],
		'msg'   => $errs[3]
	);
}
...
function db_error() {
	// Used internally for code.  Not recommended for direct printing.
	global $dbconn, $glob;
...
function db_print_error($query = false) {
	// Used for printing diagnostic errors.
	// Keep query false for security.  Set query to true for owners/debugging.
	$errs = db_error_array();
	if (empty($errs) || $GLOBALS['quiet_mode']) {
		return;
	}

	echo "<strong>db: ".$errs['trace']." &gt; ".$errs['msg']."</strong><br />\n";
	if ($query == true) {
		echo "<strong>q: `".$errs['sql']."`</strong><br />\n";
	}
}
