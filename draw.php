<?php
/*
Wacintaki Poteto - Copyright 2004-2023 Marc "Waccoon" Leveille
https://www.NineChime.com/products/
Version 1.6.10 - Last modified 2023-07-22
*/
...
if (isset($_COOKIE['drawcfg'])) {
	$drawcfg = array();
	$drawtmp = w_gpc('drawcfg', 'raw');
	if ($drawtmp !== null) {
		$drawtmp = preg_replace("/[^a-zA-Z0-9s_=;]/", '', $drawtmp);
...
if (!isset($drawcfg)) {
	// Default app is ChickenPaint (5)
	$drawcfg = array('app'=>5, 'anim'=>0, 'close'=>1);
}
...
if (isset($_REQUEST['applet'])) {
	...
	if ($drawcfg['app'] > 0) {
		$extra_cgi .= '&datatype='.$drawcfg['app'];
	}
...
<?php if (file_exists('chickenpaint/')) { ?>
					<input type="radio" name="applet" value="chickenpaint" <?php if($drawcfg['app']==5) echo $ch;?>/>ChickenPaint (HTML5)<br />
<?php } ?>
<?php if ($use_java && file_exists('spainter_all.jar')) { ?>
					<input type="radio" name="applet" value="shi" <?php if($drawcfg['app']==2) echo $ch;?>/>Shi-Painter<br />
					<input type="radio" name="applet" value="shipro" <?php if($drawcfg['app']==3) echo $ch;?>/>Shi-Painter Pro<br />
<?php } ?>
<?php if ($use_java && file_exists('chibipaint.jar')) { ?>
					<input type="radio" name="applet" value="chibipaint" <?php if($drawcfg['app']==4) echo $ch;?>/>Chibi Paint <?php tt('draw_no_anim');?><br />
<?php } ?>
<?php if ($use_java && file_exists('oekakibbs.jar')) { ?>
					<input type="radio" name="applet" value="oekakibbs" <?php if($drawcfg['app']==1) echo $ch;?>/>OekakiBBS<br />
<?php } ?>
<?php if ($use_java && file_exists('PaintBBS.jar')) { ?>
					<input type="radio" name="applet" value="notebbs" <?php if($drawcfg['app']==6) echo $ch;?>/>PaintBBS<?php tt('withpalet');?><br />
					<input type="radio" name="applet" value="paintbbs" <?php if($drawcfg['app']==0) echo $ch;?>/>PaintBBS<br />
<?php } ?>
				</p>
